...
#
# CommandShowstats - /showstats - Shows the text stats report to others on chat.
#
...
###FUTURE: 6=Planeswalker 7=Tribal?
  ...
  cardtypes = (CountAttr(curdeck, "type", "Artifact"), CountAttr(curdeck, "type", "Creature"), CountAttr(curdeck, "type", "Enchantment"), CountAttr(curdeck, "type", "Instant"), CountAttr(curdeck, "type", "Land"), CountAttr(curdeck, "type", "Sorcery"));

# cardcolors 0=White 1=Blue 2=Black 3=Red 4=Green 5=Multicolor

  cardcolors = (CountAttr(curdeck, "color", "White"), CountAttr(curdeck, "color", "Blue"), CountAttr(curdeck, "color", "Black"), CountAttr(curdeck, "color", "Red"), CountAttr(curdeck, "color", "Green"), CountAttr(curdeck, "color", " "));

# CMC curve should ignore lands
  curdeck=select("find(\"Land\",Attr(\"type\",#)) == NULL",curdeck);

  manacurve=(CountMtgManaEqual(curdeck,0),CountMtgManaEqual(curdeck,1),CountMtgManaEqual(curdeck,2),CountMtgManaEqual(curdeck,3),CountMtgManaEqual(curdeck,4),CountMtgManaEqual(curdeck,5),CountMtgManaEqual(curdeck,6),CountMtgManaEqual(curdeck,7),CountMtgManaEqual(curdeck,8),CountMtgManaGreater(curdeck,8));

  Send("Say","{orange} *** Mtg Deck Statistics *** ");

  Send("Say","{154,128,77}Lands {white}" + cardtypes[4] + " (" + (cardtypes[4]*100)/totalcards + 
  ...
  Send("Say","{154,154,154}Artifacts {white}" + cardtypes[0] + " (" + (cardtypes[0]*100)/totalcards + 
  "%) || White " + cardcolors[0] + " (" + (cardcolors[0]*100)/totalcards +
  "%) || {blue} Blue {white}" + cardcolors[1] + " (" + (cardcolors[1]*100)/totalcards + 
  "%) || {77,77,77} Black {white}" + cardcolors[2] + " (" + (cardcolors[2]*100)/totalcards + 
  ...
  Send("Say","{hr}{orange}Mana Curve {red}[ Average: {yellow}" + format("%0.2f",MtgAverageMana(curdeck)) + 
